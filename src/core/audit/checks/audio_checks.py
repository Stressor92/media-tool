    ...
    def run(
        ...
        for f in files:
            ...
            unlabeled = [
                ...
                if (s.get("tags") or {}).get("language", "und").lower() in _UNDEFINED
            ]
            if unlabeled:
                findings.append(
                    AuditFinding(
                        ...
                        path=f,
                        message=(
                            f"{len(unlabeled)} von {len(audio_streams)} "
                            "Audiospuren ohne Sprachkennzeichnung."
                        ),
                        details={"unlabeled_count": len(unlabeled)},
                        suggested_command=(
                            f'media-tool audio tag "{f}" --detect-language'
                        ),
                    )
                )
        return findings
