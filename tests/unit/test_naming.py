...
class TestFormatSeriesEpisode:
    ...
    def test_format_series_basic(self):
        ...
        assert format_series_episode("Breaking Bad", 1, 1) == "Breaking Bad - S01E01"

    def test_format_series_with_episode_title(self):
        """Should append episode title with ' - ' separator."""
        assert format_series_episode("Breaking Bad", 1, 1, "Pilot") == "Breaking Bad - S01E01 - Pilot"

    def test_format_series_multiple_digits(self):
        """Should zero-pad season and episode to 2 digits."""
        assert format_series_episode("Game of Thrones", 5, 9, "The Bells") == "Game of Thrones - S05E09 - The Bells"

    def test_format_series_string_season_episode(self):
        """Should handle season and episode as strings (numeric)."""
        assert (
            format_series_episode("Show", "2", "5") == "Show - S02E05"  # Converts strings to formatted season/episode
        )
    ...
    def test_format_series_sanitizes_title(self):
        ...
        assert format_series_episode("Show: The Title", 1, 1) == "Show - The Title - S01E01"
    ...
    def test_format_series_empty_episode_title(self):
        ...
        assert format_series_episode("Show", 1, 1, "") == "Show - S01E01"
    ...
    def test_format_series_type_coercion(self):
        ...
        assert format_series_episode("Show", 3, 7) == "Show - S03E07"
...
class TestGenerateEpisodeFilePath:
    ...
    def test_generate_episode_file_sanitizes_names(self, tmp_path):
        ...
        result = generate_episode_file_path("Show: Bad<Name>", 1, 1, "mkv", tmp_path, "Episode: Test>Title")
        # Should not contain invalid characters
        assert "<" not in str(result)
        assert ">" not in str(result)
