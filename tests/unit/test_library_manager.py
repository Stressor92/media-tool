...
class TestRefresh:
    def test_refresh_all_triggers_post(
    ...
    def test_refresh_all_returns_failed_on_exception(
        self, manager: LibraryManager, mock_client: MagicMock
    ) -> None:
    ...
    def test_refresh_library_calls_correct_endpoint(
        self, manager: LibraryManager, mock_client: MagicMock
    ) -> None:
    ...
    def test_refresh_item_default_params_no_replace(
        self, manager: LibraryManager, mock_client: MagicMock
    ) -> None:
...
class TestScanStatus:
    ...
    def test_scan_status_idle_when_no_matching_task(
        self, manager: LibraryManager, mock_client: MagicMock
    ) -> None:
    ...
    def test_scan_status_failed_on_exception(
        self, manager: LibraryManager, mock_client: MagicMock
    ) -> None:
    ...
    def test_wait_for_scan_returns_immediately_when_idle(
...
class TestFindLibraryForPath:
    def test_find_library_matches_correct_library(
        self, manager: LibraryManager, mock_client: MagicMock
    ) -> None:
        mock_client.get.return_value = [
            {
        ...
        result = manager._find_library_for_path(
            Path("/media/movies/Inception (2010)")
        )
        assert result is not None
        assert result.name == "Movies"

    def test_find_library_returns_none_when_no_match(
        self, manager: LibraryManager, mock_client: MagicMock
    ) -> None:
        mock_client.get.return_value = [
            {
    ...
    def test_refresh_path_falls_back_to_refresh_all(
        self, manager: LibraryManager, mock_client: MagicMock
    ) -> None:
        mock_client.get.return_value = []
